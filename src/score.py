...
def re_score2(pred_relations, gt_relations):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> SCORE RE")
    """Evaluate RE predictions
    Args:
        pred_relations (list) :  list of list of predicted relations (several relations in each sentence)
    ...
    #n_rels = sum([len([rel for rel in sent]) for sent in pred])
    #n_found = sum([len([rel for rel in sent]) for sent in gold])

    # relation_types = [v for v in relation_types if not v == "None"]

    
    relations_all=["rdfs:label","dbo:birthDate","dbo:deathDate","subj"]
    scores = {rel: {"tp": 0, "fp": 0, "fn": 0} for rel in relations_all + ["ALL"]}
    

    n_sents = len(gt_relations)
    relations_only=["rdfs:label","dbo:birthDate","dbo:deathDate"]
    nb_rel = []
    nb_found = []
    n_subj_ok = 0
    
    for p_sent, g_sent in zip(pred_relations, gt_relations):
        rel_found = []
        rel_gold = []

        regex_subj="dbr:(\w*)"
        r_pred =  re.findall(regex_subj,p_sent)
        r_gold =  re.findall(regex_subj,g_sent)
        
        if(r_gold and r_pred):
            
            if(r_gold[0]==r_pred[0]):
                n_subj_ok += 1
                scores["subj"]["tp"] += 1
            else:
                scores["subj"]["fp"] += 1
        ...
        for rel in relations_only:
            regex=rel.replace(":","\:")+' \"(.*?)\"'
            r_pred = re.findall(regex,p_sent)
            r_gold =  re.findall(regex,g_sent)
            if(r_gold):
                rel_gold.append(rel)
            if(r_gold and r_pred):
                if(r_gold[0]==r_pred[0]):
...
def re_score3(pred_relations, gt_relations):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> SCORE RE")
    """Created because of dbr:subject problem with double <s><s> """
    ...
    for p_sent, g_sent in zip(pred_relations, gt_relations):
        rel_found = []
        rel_gold = []
        if("br:" in p_sent and "dbr:" not in p_sent):
            p_sent=p_sent.replace("br:","dbr:")
        regex_subj="dbr:(\w*)"
        r_pred =  re.findall(regex_subj,p_sent)
        r_gold =  re.findall(regex_subj,g_sent)
...
def toListRel(txt,format_):
    ...
    try: 
        ...
        if(format_ in ["json-ld","turtle","xml","ntriples"]):
            ...
            list_rel=[stmt for stmt in g1]

        elif(format_ == "list"):
            if(txt[0:2]!="((" and txt[0:1]=="("):
                txt="("+txt
            sub_list=txt.split('), (')
            for i in range(len(sub_list)):
                sub_list[i]=sub_list[i].replace("(","").replace("\n","").replace(")","")
                sub_list[i]=sub_list[i].split("', '")
            list_rel=sub_list
        elif(format_ == "tags"):
            if(txt[0:1]!="<"):
                txt="<"+txt
            temp=txt.replace("<subj>","").replace("<rel>",";").replace("<obj>",";").split("<et>")[:-1]
            for t in temp:
                list_rel.append(t.split(";"))
        ...
        print("ERROR DURING PARSING >",format_," :")
        print(txt)
        print("-------------")
        return list_rel
    return list_rel

def re_score4(pred_relations, gt_relations,format_):
    print("+++++++++++++++++++++++++++++++++++++++ SCORE4")
   
    relations_all=["label","birthDate","deathDate","birthYear","deathYear"]
    scores = {rel: {"tp": 0, "fp": 0, "fn": 0} for rel in relations_all + ["ALL"]}
    ...
    for p_sent, g_sent in zip(pred_relations, gt_relations):
        ...
        list_rel_gt=toListRel(g_sent,format_)
        list_rel_pred=toListRel(p_sent,format_)
        # print("LIST REL==============")
        # print(list_rel_pred)
        # print("--------------")
        # print(list_rel_gt)
        ...
        if(isinstance(list_rel_gt, list) and len(list_rel_gt)>0):
        ...
        if(isinstance(list_rel_pred, list) and len(list_rel_pred)>0):
            print("--list_rel_pred ok")
    ...
    n_rels = sum(nb_rel)
    n_found = sum(nb_found)
    print(">>>>>>>>>>>>>>>>>>>>>>> NB OF PARSING OK >",n_parsed,"/",n_sents)
    print(">>>>>>>>>>>>>>>>>>>>>>> NB OF SUBJ OK >",n_subj_ok,"/",n_sents)
    print(
        "processed {} sentences with {} relations; found: {} relations; correct: {}.".format(n_sents, n_rels, n_found,
                                                                                             tp))
    print(
...
def tl_score(predictions,labels, verbose=False):
