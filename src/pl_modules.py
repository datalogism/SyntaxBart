...
from score import score, re_score, re_score2, re_score3, re_score4
...
relation_dbpedia = {
    "label","birthDate","birthYear","birthName","deathDate","deathYear","alias","type"
}
...
class BasePLModule(pl.LightningModule):
    ...
    def generate_triples(self,
        ...
        decoded_labels = self.tokenizer.batch_decode(torch.where(labels != -100, labels, self.config.pad_token_id), skip_special_tokens=True, truncation=True)

        if self.hparams.dataset_name.split('/')[-1] == 'dbpedia_test.py':
    ...
    def generate_samples(self,
        ...
        decoded_preds = self.tokenizer.batch_decode(generated_tokens[relation_start==1], skip_special_tokens=True, truncation=True)
    ...
    def validation_epoch_end(self, output: dict) -> Any:
        ...
        elif "dbpedia" in self.hparams.dataset_name:
            print("HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE")
            if("json-ld" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"json-ld")
            elif("list" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"list")
            elif("turtleS" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"turtleS")
            elif("ntriples" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"ntriples")
            elif("turtle" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"turtle")
            elif("tags" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"tags")
            elif("xml" in self.hparams.train_file):
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']],"xml")
                #scores, precision, recall, f1 = re_score2([pred['predictions'] for pred in output ], [pred['labels'] for pred in output ])
            else:
                scores, precision, recall, f1 = re_score4([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']])
                #scores, precision, recall, f1 = re_score2([pred['predictions'] for pred in output ], [pred['labels'] for pred in output ])
            self.log('val_prec_micro', precision)
            self.log('val_recall_micro', recall)
            self.log('val_F1_micro', f1)
            print("-----------")
            print('val_prec_micro', precision)
            print('val_recall_micro', recall)
            print('val_F1_micro', f1)
            print("-----------")
